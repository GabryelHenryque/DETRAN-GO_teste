...
container_quest.innerHTML += `
    <div class="text-center container "id="container_button">
...
function startResult() {
    ...
    //texto titulo
    const title_id = getId('title')
    title_id.textContent = "RESULTADO";

    //texto resultado
    const point_id = getId('point_id')
    const contmax = Math.round((75/100) * maxleng);
    point_id.textContent = `resultado: ${point}/${maxleng}`
    point_id.style.color = point < contmax ?  "red" : "green";
    // scrola para cima
    setTimeout(() => {
    ...
    //muda botão
    let container_button = getId('container_button')
    container_button.innerHTML = `
      <button type="button" class="btn btn-outline-secondary" onclick="refresh()">fazer teste de novo</button>  
    `
}
